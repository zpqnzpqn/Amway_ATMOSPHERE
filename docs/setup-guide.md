...
3. 國家代碼預設為 `TW`（台灣）。
4. 點選 **「傳送」**，系統將在 2 秒內自動完成 App 鑑權交換，並自動取得永久 Refresh Token！
5. 清淨機設備與感測器將立即出現在 Home Assistant！

---

## 🔗 備用連線方式二：官方 OAuth 授權跳轉碼

若您偏好透過瀏覽器手動登入安麗官方網頁授權：

1. 點擊對話框中的官方連結：**`👉 點此前往安麗官方登入頁面`**。
2. 在瀏覽器中正常登入您的安麗帳號與密碼。
3. 登入成功後，瀏覽器網址列會嘗試跳轉至 `amwayhealthyhome://loginRedirect?code=xxxxxxxx...`。
4. 複製網址列的**完整網址（或 code 代碼）**，貼回 Home Assistant 的「官方授權跳轉碼 (Code)」欄位。
5. 點選 **「傳送」** 即完成！
